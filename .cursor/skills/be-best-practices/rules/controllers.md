...
### 统一响应 schema 使用 envelope

Controller 的 `response` 需要声明 envelope model（如 `user.responseList`），不要继续声明裸领域对象（如 `user.list`）。
...
**❌ 错误写法：**
```ts
// 继续使用裸领域对象，导致 OpenAPI 与真实响应不一致
.get('/', () => UserService.findAll(), { response: 'user.list' })
.post('/', ({ body }) => UserService.create(body), { response: 'user.item' })

// 在 handler 内手工包裹成功 envelope（应由全局 plugin 处理）
.get('/', async () => ({
  code: 0,
  msg: 'ok',
